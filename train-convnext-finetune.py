...
def collect_samples(train_folder, min_images_per_class):
    ...
    class_to_idx = {
        class_name: index
        for index, class_name in enumerate(class_to_paths)
    }
...
for epoch in range(1, epochs + 1):
    train_backbone = epoch > freeze_epochs
    set_backbone_trainable(model, trainable=train_backbone)

    if train_backbone:
        print(f"Epoch {epoch}: fine-tuning full ConvNeXt model")
    else:
        print(f"Epoch {epoch}: training classifier head only")
